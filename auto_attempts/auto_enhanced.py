#!/usr/bin/env python3
"""
AHRQ Enhanced Analysis Engine (v3.0)
Using OpenRouter API with Gemini 2.5 Flash
...
class AHRQ_OpenRouter_Analysis_Pipeline:
    def __init__(self, scopus_api_key: str, openrouter_api_key: str):
        ...
        self.client = OpenAI(
            ...
            default_headers={
                "HTTP-Referer": "https://ahrq-analysis.com",
                "X-Title": "AHRQ Analysis Engine"
            }
        )
    ...
    def _validate_openrouter_key(self):
        """Validates the OpenRouter API key."""
        print("--- Validating OpenRouter API Key ---")
        # Skip validation for now - will validate on first real request
        print("✅ OpenRouter API Key configured (validation on first use).")
...
{document_text}"""
        ...
        try:
            ...
            for para in root.findall('.//ce:para', namespaces):
                if para.text:
                    ...
                    if 'AHRQ' in text or 'Agency for Healthcare Research' in text or 'Compendium' in text:
    ...
    def analyze_with_openrouter(self, document_content: Dict[str, str]) -> dict:
        """Sends structured content to OpenRouter for analysis."""
        print("      Sending to Gemini 2.5 Flash via OpenRouter for analysis...")
...
AHRQ/COMPENDIUM MENTIONS FOUND:
{chr(10).join(document_content.get('ahrq_mentions', ['None found']))}

FULL TEXT:
{document_content.get('body', 'Not found')}
"""
        ...
        try:
            response = self.client.chat.completions.create(
                extra_headers={
                    "HTTP-Referer": "https://ahrq-analysis.com",
                    "X-Title": "AHRQ Enhanced Analysis Engine"
                },
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'error' in extracted_content:
                    ...
                    analysis = self.analyze_with_openrouter(extracted_content)
                    ...
                    if 'error' in analysis:
                        ...
                        if extracted_content.get('doi'):
                            analysis['DOI_URL'] = extracted_content['doi']
                        if extracted_content.get('year'):
                            analysis['Publication_Year'] = extracted_content['year']
                        if extracted_content.get('journal'):
...
if __name__ == "__main__":
    ...
    try:
        pipeline = AHRQ_OpenRouter_Analysis_Pipeline(
